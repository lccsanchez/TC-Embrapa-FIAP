...
def mock_urls(monkeypatch):
    monkeypatch.setattr(
        ...
        "sessions",
        {
            "importacao": {"item": "opt_05", "sub": {"ImpVinhos": "subopt_01"}},
...
def test_find_with_subitems_success(monkeypatch, mock_urls):
    ...
    monkeypatch.setattr(
        ...
        lambda self, html: called.setdefault("scraped", {"data": "subitem ok"}),
    )

    result = scapper_repository.find_with_subitems("2023", "importacao", "ImpVinhos")
...
def test_find_with_justitems_success(monkeypatch, mock_urls):
    ...
    monkeypatch.setattr(
        ...
        lambda self, html: called.setdefault("scraped", {"data": "justitem ok"}),
    )
